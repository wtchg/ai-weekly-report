...
def generate_report_content() -> str:
    ...
    prompt = f"""
今天是 {current_date}。你是一名在同花顺（金融科技公司）移动互联网事业部工作的数据分析师，主要职责是移动端新用户增长分析与平台商业分析。你未来希望进入纯互联网行业（字节/淘天/小红书/腾讯等），所以需要同时关注互联网行业的整体趋势。

请撰写本周的《AI与数据驱动周报》。受众是你的同事（数据分析师和产品运营）。

【内容约束】
1. 只写本周（{current_date}所在周）真实发生的行业动态。不确定或无法核实的消息不要写。涉及非公开的具体数据指标时（如"DAU提升22%"），必须标注来源或说明"基于公开报道推算"。
...
【第二部分特别要求】
6. 第二部分聚焦字节、淘天、小红书、腾讯等头部互联网公司的数据实践和AI生态动态，不要局限于金融科技。AI基础设施部分要关注本周的模型发布、开源动态、API能力更新、行业政策等。

【大厂社招分析部分特别要求】
7. 第三部分必须基于{current_date}前后字节跳动、淘天集团、阿里国际、小红书、腾讯的真实或高度可信的社招动态，分析数据类、策略产品类、AI产品类、运营类岗位的JD趋势。
8. 必须覆盖至少2家公司的具体岗位需求，给出核心技能的变化方向。
9. 差距分析必须与用户的当前岗位（新用户数据分析师 / 平台商业分析师）对齐，给出可操作的提升建议。

【技能精进部分特别要求】
10. 第四部分推荐的技能方向应是高阶方向，排除基础 SQL（SQL已是基线能力，不应出现）。建议方向：因果推断方法（DID/RDD/IV）、AB实验设计与MDE计算、Python数据管道自动化、LLM在数据分析工作流中的应用、推荐系统理解、时间序列异常检测、数据产品设计。
11. "4.1 是什么"部分必须用1-2句大白话解释该概念，假设读者第一次听说——不要只写定义，要让人读完就懂它解决什么问题。
12. "4.2 行业中的真实用例"部分必须给出1个互联网/金融科技公司实际使用该方法的案例，说明什么场景下用、解决了什么问题。没有确切来源就标注"基于公开报道推算"。
13. "4.3 学习路径"必须具体：给出一篇可检索的文章标题或关键词、一个能在一周内完成的练习、一个在当前数据分析工作中直接可用的场景。
